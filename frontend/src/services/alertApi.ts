    ...
    try {
      ...
      if (res.ok) {
        ...
        return (data || []).map((a: any) => ({
          ...
          message: a.reason || `${a.hazard} emergency alert in Chennai sector.`,
          locationName: a.location?.name || 'Chennai Operational Zone',
          coordinates: [a.location?.latitude || 13.04, a.location?.longitude || 80.23] as [number, number],
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          ...
          message: a.reason,
          locationName: a.location?.name || 'Chennai Operational Zone',
          coordinates: [a.location?.latitude || 13.04, a.location?.longitude || 80.23],
