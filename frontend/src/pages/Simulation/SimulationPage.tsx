  ...
  return (
    <div className="flex w-full h-full bg-[#0B0F19] text-slate-100 overflow-hidden select-none">
      ...
      <div className="w-[410px] bg-[#0E1526] border-r border-slate-800 p-4 h-full flex flex-col justify-between overflow-y-auto shrink-0 space-y-4 text-xs font-mono">
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <span>{weatherLoading ? 'FETCHING...' : (liveWeatherLoaded ? 'LIVE WEATHER LOADED' : 'PREFILL LIVE WEATHER')}</span>
              </button>
            </div>
        ...
        {/* 360-Degree Directional ML Probability Spectrum (All Possible Directions) */}
        {directedResult && directedResult.directionalSpectrum?.length > 0 && (
          <div className="mt-4 pt-3 border-t border-slate-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[10px] text-cyan-400 tracking-wider uppercase flex items-center space-x-1.5">
                <Compass className="w-3.5 h-3.5 text-cyan-400" />
                <span>ALL PREDICTED DIRECTIONS (360° SPECTRUM)</span>
              </span>
            ...
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-1">
              {directedResult.directionalSpectrum.map((dir: any) => {
                ...
                const barColor = isPrimary
                  ? 'bg-rose-500'
                  : (isSecondary ? 'bg-amber-500' : (prob >= 30 ? 'bg-cyan-500' : 'bg-slate-600'));
                const textColor = isPrimary
                  ? 'text-rose-400'
                  : (isSecondary ? 'text-amber-400' : (prob >= 30 ? 'text-cyan-400' : 'text-slate-400'));
                  ...
                  <div
                    ...
                    className={`p-2 rounded-xl border text-[10px] font-mono transition-all ${
                      isPrimary
                        ...
                        : (isSecondary ? 'bg-amber-950/20 border-amber-500/40' : 'bg-slate-950/60 border-slate-800/80')
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <div
                          className="w-4 h-4 rounded-full bg-slate-800 flex items-center justify-center text-[10px] text-slate-300"
                          style={{ transform: `rotate(${dir.bearingDeg}deg)` }}
                        >
                          ↑
                        </div>
                        <span className="font-bold text-slate-200">
                          {dir.direction} ({dir.bearingDeg}°)
                        </span>
                        {isPrimary && (
                          <span className="px-1 py-0.2 rounded text-[8px] font-bold bg-rose-500/20 text-rose-300 border border-rose-500/40">
                            PRIMARY
                          </span>
                        )}
                        {isSecondary && (
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-[9px] text-slate-400">{dir.etaText}</span>
