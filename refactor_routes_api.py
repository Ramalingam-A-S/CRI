...
routes_py_path = r'd:\Aracnids\backend\api\routes.py'
...
def analyze_route(req: RouteRequest):
    ...
    # We want to find a Safer alternative
    # Iterate alternatives to find one that has a lower climate score
    alternatives = [r for r in evaluated_routes[1:] if r["summary"]["climate_score"] < fastest["summary"]["climate_score"] - 5.0]
    if alternatives:
        # Pick the one with the lowest climate score
        safer = min(alternatives, key=lambda r: r["summary"]["climate_score"])
...
with open(routes_py_path, 'w', encoding='utf-8') as f:
    f.write(content)
